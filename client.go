...
// Client is a coinbase websocket client.
type Client struct {
	ws        *websocket.Conn
	output    chan string
	connected bool
...
// NewClient returns a new instance of coinbase websocket client.
...
func (c *Client) Run() {
	for {
		...
		c.ws, _, c.err = websocket.DefaultDialer.Dial(url, nil)
		if c.err != nil {
			log.Println(c.err)
			select {
			case <-c.quit:
